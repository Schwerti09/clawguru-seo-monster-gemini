...
export function middleware(request: NextRequest) {
  ...
  // SEO redirect (301): migrate legacy OpenClaw runbook URLs to canonical city URLs.
  // Supports both /runbook/openclaw-risk-2026-{city} and /{lang}/runbook/openclaw-risk-2026-{city},
  // preserves incoming locale when valid, defaults to /de, normalizes city slug to lowercase,
  // and avoids redirect loops by only matching legacy /runbook/* path shapes.
  const legacyOpenclawRisk = pathname.match(/^\/(?:([a-z]{2}(?:-[a-z]{2})?)\/)?runbook\/openclaw-risk-2026-([a-z0-9-]+)\/?$/i)
  if (legacyOpenclawRisk) {
    const localeFromPath = legacyOpenclawRisk[1]?.toLowerCase() as Locale | undefined
    const targetLocale = localeFromPath && SUPPORTED_LOCALES.includes(localeFromPath) ? localeFromPath : DEFAULT_LOCALE
    const city = legacyOpenclawRisk[2].toLowerCase()
