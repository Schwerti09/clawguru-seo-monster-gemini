...
export default function ShareUnlockPanel({ title, slug, items, previewCount = 3 }: Props) {
    ...
    <div>
      ...
      {!unlocked && lockedItems.length > 0 && (
        <div className="mt-6 rounded-3xl border border-cyan-500/30 bg-cyan-500/10 p-5 space-y-4">
          <div className="text-sm text-cyan-200 font-bold">Unlock {lockedItems.length} extra steps</div>
          ...
          <form onSubmit={handleEmailSubmit} className="flex flex-col sm:flex-row gap-2">
            ...
            >
              {status === "loading" ? "Unlocking…" : "Unlock via Email"}
            </button>
          </form>
          {status === "error" && (
            <div className="text-xs text-red-300">Bitte eine gültige Email-Adresse angeben.</div>
          )}
          {status === "success" && (
            <div className="text-xs text-emerald-300">Unlocked. Viel Erfolg mit dem Runbook!</div>
          )}
        </div>
      )}
    </div>
